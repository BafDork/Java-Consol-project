...
public class Game extends Addition {
    ...
    private void dispatcher(String action) {
        switch (action) {
            ...
            case "6", "inventory" -> inventory.main();
    ...
    public void loadLevel(boolean newGame) {
        boolean successLoad = map.loadMap(newGame);
        if (successLoad) {
    ...
    private void checkPosition() {
        if (map.getBuffer() == 'e') {
            map.setBuffer('1');
            map.newLevel();
            if (map.getLevelNum() <= map.getLevelMax()) {
                loadLevel(true);
                main();
            }
            else {
        ...
        if (map.getBuffer() == 'f') {
            new Fight(enemy.generateEnemy());
        }
        if (map.getBuffer() == 'c') {
            chest.openChest(hero.getX() + ":" + hero.getY());
        }
    }
